 ...
 * author : kk-小德
 * date : 2018/12/27
 * 原项目地址https://github.com/koudle/AndroidSiriWave.git
 **/
public class SiriWaveView extends View {

    public static final String TAG = "SiriWaveView";
    ...
    //颜色
//    private int[] COLORS = {R.color.green, R.color.blue, R.color.pink,R.color.white};
...
//                Log.e("AAA-->", "initAnimator: " + waveBean.waveHeight);
            }
        });
...
//        }
        ...
        public void respawn() {
            ...
            int realAmplitude = getRealAmplitude(color);
            this.seed = Math.random();  // 位置
            maxWidth = (random.nextInt(mWidth / 16) + mWidth * 3 / 11);
            if (seed <= 0.2) {
                maxHeight = random.nextInt(realAmplitude / 4) + realAmplitude / 5;
                open_class = 2;
            } else if (seed <= 0.3) {
                maxHeight = random.nextInt(realAmplitude / 3) + realAmplitude * 1 / 5;
                open_class = 3;
            } else if (seed <= 0.7) {
                maxHeight = random.nextInt(realAmplitude / 2) + realAmplitude * 2 / 5;
                open_class = 3;
            } else if (seed <= 0.8) {
                maxHeight = random.nextInt(realAmplitude / 3) + realAmplitude * 1 / 5;
                open_class = 3;
...
//            color = COLORS[random.nextInt(3)];
        ...
        public int getRealAmplitude(int color){
            float a = 1f;
            switch (color){
                case R.color.wave_blue:
                    a = 1f;
                    break;
                case R.color.wave_pink:
                    a = 34f /48f;
                    break;
                case R.color.wave_green:
                    a = 25f /48f;
                    break;
                case R.color.wave_white:
                    a = 13f /48f;
                    break;
            }
            int realAmplitude = (int) (a * MAX_AMPLITUDE);
//            Log.d(TAG,"getRealAmplitude--a: " + a + " realAmplitude； " + realAmplitude);
